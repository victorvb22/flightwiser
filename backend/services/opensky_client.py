...
class TokenManager:
    ...
    @classmethod
    def from_settings(cls) -> "TokenManager | None":
        """Create a :class:`TokenManager` from Flightwiser's env-based config.

        Returns None if no OpenSky credentials are configured (falls back to
        anonymous access, with reduced rate limits).
        """
        if settings.opensky_client_id and settings.opensky_client_secret:
            return cls(settings.opensky_client_id, settings.opensky_client_secret)
        return None
...
class OpenSkyApi:
    ...
    needed. The recommended pattern is to use it as a context manager::
        ...
        with OpenSkyApi(token_manager=TokenManager.from_settings()) as api:
    ...
    def from_settings(cls) -> "OpenSkyApi":
        """Build a client using Flightwiser's env-based config (see config.py)."""
        return cls(token_manager=TokenManager.from_settings())
